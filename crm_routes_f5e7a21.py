...
def register_crm_routes(app):
    ...
    def update_prospect_from_report(prospect_id):
        ...
        try:
            ...
            if 'data_json' in data:
                ...
                params.append(json.dumps(data['data_json']))
                    ...
                    if existing_project:
                        ...
                        execute_query('''
                            ...
                            SET data_json = %s, 
                                ...
                                commune = %s,
                                adresse_projet = %s,
                                parcelles_cadastrales = %s
                            WHERE id = %s
                        ''', (
                            json.dumps(data['data_json']),
                            data.get('commune'),
                            data.get('commune'),  # Utiliser commune comme adresse_projet si pas d'adresse
    ...
    def page_calpinage_pv(prospect_id):
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute("SELECT * FROM agriweb_prospects WHERE id = %s", (prospect_id,))
    ...
    def save_calpinage(prospect_id):
        ...
        try:
            ...
            return jsonify({'success': True, 'message': 'Calpinage sauvegardé'})
    ...
    def generer_etude_productible(prospect_id):
        """Générer un PDF d'étude de productible"""
        try:
            ...
            # Notes et hypothèses
            y = y - len(table_data) * 0.7*cm - 2*cm
            c.setFont("Helvetica-Bold", 12)
            c.drawString(2*cm, y, "📝 HYPOTHÈSES DE CALCUL")
            y -= 0.7*cm
            
            c.setFont("Helvetica", 9)
            dept = prospect.get('departement', '23')
            ensoleillement = {'23': 1100}.get(dept, 1100)
            
            c.drawString(2.5*cm, y, f"• Ensoleillement moyen (département {dept}): {ensoleillement} kWh/kWc/an")
            y -= 0.5*cm
            c.drawString(2.5*cm, y, "• Coefficient d'orientation: variable selon l'azimut (optimal au Sud)")
            y -= 0.5*cm
            c.drawString(2.5*cm, y, "• Coefficient d'inclinaison: optimal entre 25° et 35°")
            y -= 0.5*cm
            c.drawString(2.5*cm, y, "• Pertes système (câblage, onduleur, etc.): incluses dans les coefficients")
