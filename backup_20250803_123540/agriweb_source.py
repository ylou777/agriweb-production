...
def get_commune_mairie(nom_commune):
    ...
    if resp.status_code == 200:
        info = resp.json()
        if info and len(info) > 0 and "mairie" in info[0]:
    ...
    def collect_context_data():
        ...
        try:
            ...
            try:
                ...
                if commune_name and commune_name != "N/A" and commune_name.strip():
                    ...
                    # Recherches spécifiques pour chaque type de service
                    recherches_services = []
                    
                    # 1. Mairie de la commune
                    recherches_services.append(f'nom = "Mairie - {commune_name}"')
                    
                    # 2. DREAL Pays de la Loire
                    recherches_services.append('nom = "Direction régionale de l\'environnement, de l\'aménagement et du logement (DREAL) - Pays de la Loire"')
                    
                    # 3. DDTM Loire-Atlantique (pour les communes côtières de Loire-Atlantique)
                    recherches_services.append('nom = "Direction départementale des territoires et de la mer (DDTM) - Loire-Atlantique"')
                    
                    # 4. Chambre d'agriculture Loire-Atlantique
                    recherches_services.append('nom = "Chambre d\'agriculture - Loire-Atlantique"')
                    
                    # Combiner toutes les recherches avec OR
                    where_clause = ' OR '.join([f'({req})' for req in recherches_services])
                    ...
                    if admin_response.status_code == 200:
                        ...
                        if services and len(services) > 0:
                            log_step("CONTEXT", f"🔍 API Administration: {len(services)} services bruts trouvés", "INFO")
                            
                            # Fonction de filtrage robuste pour mairie, DREAL, DDT uniquement
                            def is_service_pertinent(service):
                                """Filtre strict pour mairie, DDTM, DREAL et chambre d'agriculture uniquement"""
                                ...
                                # 1. Mairie (recherche exact "Mairie - ")
                                if 'mairie - ' in nom_lower or nom_lower.startswith('mairie '):
                                    return True
                                
                                # 2. DREAL ou Direction régionale de l'environnement  
                                if 'dreal' in nom_lower or 'direction régionale de l\'environnement' in nom_lower:
                                    return True
                                
                                # 3. DDTM uniquement (Direction Départementale des Territoires et de la Mer)
                                if 'ddtm' in nom_lower or 'direction départementale des territoires et de la mer' in nom_lower:
                                    return True
                                    
                                # 4. Chambre d'agriculture
                                if 'chambre d\'agriculture' in nom_lower or 'chambre agriculture' in nom_lower:
                                    return True
                            ...
                            for service in services:
                                try:
                                    # Vérifier si le service est pertinent
                                    nom_service = service.get('nom', 'Inconnu')
                                    if not is_service_pertinent(service):
                                        services_ignores.append(nom_service)
                                        continue
                                    
                                    log_step("CONTEXT", f"✅ Service retenu: {nom_service}", "INFO")
                                    
                                    # Parser l'adresse de façon sécurisée
                                    adresse_info = {
                                        'numero_voie': '',
                                        'code_postal': '',
                                        'commune': '',
                                    ...
                                    if adresse_raw:
                                        try:
                                            if isinstance(adresse_raw, str):
                                                ...
                                                adresses = adresse_raw
                                            else:
                                                adresses = []
                                                
                                            if adresses and len(adresses) > 0 and isinstance(adresses[0], dict):
                                                adresse_principale = adresses[0]
                                                adresse_info.update({
                                                    'numero_voie': str(adresse_principale.get('numero_voie', '')),
                                                    'code_postal': str(adresse_principale.get('code_postal', '')),
                                                    'commune': str(adresse_principale.get('nom_commune', '')),
                                                    'longitude': str(adresse_principale.get('longitude', '')),
                                                    'latitude': str(adresse_principale.get('latitude', ''))
                                                })
                                        except Exception as parse_error:
                                            log_step("CONTEXT", f"⚠️ Erreur parsing adresse: {parse_error}", "WARNING")
                                    
                                    # Construire les informations du service de façon sécurisée
                                    service_info = {
                                        'nom': str(service.get('nom', 'N/A')),
                                        'type_organisme': str(service.get('type_service', 'Service déconcentré')),
                                        'categorie': str(service.get('categorie', 'N/A')),
                                        'mission': '',
                                        'telephone': '',
                                        'email': str(service.get('email', '') or ''),
                                        'site_web': str(service.get('site_web', '') or ''),
                                        'adresse': adresse_info,
                                        'url_service_public': str(service.get('url_service_public', '') or ''),
                                        'horaires': ''
                                    }
                                    
                                    # Mission (tronquée si trop longue)
                                    mission = service.get('mission', '')
                                    if mission and isinstance(mission, str):
                                        if len(mission) > 300:
                                            service_info['mission'] = mission[:300] + '...'
                                        else:
                                            service_info['mission'] = mission
                                    
                                    # Traitement du téléphone
                                    telephone_raw = service.get('telephone', [])
                                    if telephone_raw and isinstance(telephone_raw, list) and len(telephone_raw) > 0:
                                        tel_info = telephone_raw[0]
                                        if isinstance(tel_info, dict):
                                            service_info['telephone'] = str(tel_info.get('valeur', ''))
                                    
                                    services_list.append(service_info)
                                    
                                except Exception as service_error:
                                    log_step("CONTEXT", f"⚠️ Erreur traitement service: {service_error}", "WARNING")
                                    continue
                            
                            log_step("CONTEXT", f"📊 Bilan filtrage: {len(services_list)} retenus, {len(services_ignores)} ignorés", "INFO")
                            
                            # Stocker les données
                            if services_list:
                                report_data["api_externe"]["services_publics"] = services_list
                                
                                # Compter les services avec téléphone et site web
                                services_avec_telephone = sum(1 for s in services_list if s['telephone'])
                                services_avec_site_web = sum(1 for s in services_list if s['site_web'])
                                ...
                                api_details["codes_postaux"]["details"] = {
                                    ...
                                    "services_avec_telephone": services_avec_telephone,
                                    "services_avec_site_web": services_avec_site_web,
                                    "commune": commune_name
                                }
                                
                                log_step("CONTEXT", f"✅ API Administration: {len(services_list)} services publics trouvés", "SUCCESS")
                            else:
                                api_details["codes_postaux"]["error"] = "Aucun service public pertinent trouvé (mairie, DREAL, DDTM, Chambre agriculture)"
                                log_step("CONTEXT", "⚠️ API Administration: Aucun service pertinent trouvé", "WARNING")
                        else:
                    ...
                    api_details["codes_postaux"]["error"] = "Nom de commune non disponible pour la recherche"
                    log_step("CONTEXT", "⚠️ API Administration: Commune non identifiée", "WARNING")
