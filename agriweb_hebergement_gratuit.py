...
def api_lidar_copc_roof():
    ...
    try:
        ...
        # Approximation robuste : soustraire le percentile 5 des z de classe-6
        # (les points les plus bas ≈ avant-toit) → height above eave level.
        z_arr_f = np.array(rz_f, dtype=np.float64)
        z_baseline = float(np.percentile(z_arr_f, 5))  # ≈ altitude de l'avant-toit
...
def api_lidar_roof_analysis():
    ...
    try:
        ...
        if r_bd.status_code == 200:
            ...
            if building_coords and best_dist < 80:
                ...
                print(f"✓ Bâtiment trouvé: {width_m:.0f}x{length_m:.0f}m ({width_m*length_m:.0f}m²), h={building_data.get('hauteur')}m, dist={best_dist:.0f}m")
            else:
                print(f"⚠ Aucun bâtiment trouvé à moins de 80m du clic")
    except Exception as e:
    ...
    if building_coords:
        try:
            ...
            if mns_full is not None and mnt_full is not None:
                ...
                # car les sheds peuvent avoir des noues proches du sol
                mnh_threshold = 1.0 if bldg_area_m2 > 500 else 2.0
                
                roof_points = []
                roof_grid_mask = np.zeros((final_h, final_w), dtype=bool)  # masque pour gradient
...
def build_simple_map(
    ...
    # Outils de dessin/mesure retirés (interface épurée)
...
def build_map(
    ...
    # Draw et MeasureControl retirés — interface épurée

    # Cadastre
    cadastre_group = folium.FeatureGroup(name="Cadastre (WFS)", show=False)  # Toujours caché par défaut
    if parcelle_props and parcelle_props.get("geometry"):
    ...
    sir_group = folium.FeatureGroup(name="Entreprises Sirene", show=False)  # Toujours caché par défaut
...
# NOTE: Les routes /api/crm/prospects, /api/crm/prospect/<id> et /api/crm/stats
# sont définies dans crm_routes.py avec support multi-tenant + PostgreSQL.
